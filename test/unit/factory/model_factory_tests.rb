...
class MR::Factory::ModelFactory
  ...
  class InstanceTests < UnitTests
    setup do
      ...
      Assert.stub(MR::Factory::RecordFactory, :new).with(@record_class) do
        @record_factory
      end
  ...
  class InstanceMethodTests < InstanceTests
    ...
    setup do
      ...
      Assert.stub(@record_factory, :instance){ @record }
    end
  ...
  class SavedInstanceMethodTests < InstanceTests
    ...
    should "build an instance of the model and save it" do
      model = subject.instance
      Assert.stub(subject, :instance){ model }
  ...
  class InstanceStackTests < InstanceTests
    ...
    setup do
      ...
      Assert.stub(@factory, :instance).with(nil){ @model }

      @model_stack = 'a-model-stack'
      Assert.stub(MR::Factory::ModelStack, :new).with(@model){ @model_stack }

      @model_with_args = @model_class.new.tap(&:save)
      Assert.stub(@factory, :instance).with(:name => 'test'){ @model_with_args }

      @args_model_stack = 'a-model-stack-with-args'
      Assert.stub(MR::Factory::ModelStack, :new).with(@model_with_args) do
        @args_model_stack
      end
    end
